...
import { getCharacterStage, getXPForNextLevel, LEVEL_THRESHOLDS } from "@/lib/gameStore";
...
export default function CharacterDisplay({ gameState, isLevelingUp }: Props) {
  const stage = getCharacterStage(gameState.level);
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      <div className="relative">
        <div
          className={`w-36 h-36 rounded-2xl bg-gradient-to-b ${stage.color} flex items-center justify-center
        ...
        >
          <span
            className={`text-7xl select-none drop-shadow-lg transition-all duration-300
              ${isLevelingUp ? "scale-110" : "scale-100"}`}
          >
            {stage.emoji}
          </span>
          {hat && (
            <span className="absolute -top-5 text-3xl drop-shadow">{hat.emoji}</span>
          )}
          {weapon && (
            <span className="absolute -right-5 top-4 text-3xl drop-shadow">{weapon.emoji}</span>
          )}
          {accessory && (
            <span className="absolute -left-5 top-4 text-3xl drop-shadow">{accessory.emoji}</span>
          )}
            ...
            <div className="absolute inset-0 pointer-events-none">
              {["⭐", "✨", "💫", "⚡", "🌟"].map((s, i) => (
                <span
                  ...
                  style={{
                    top: `${10 + Math.sin(i * 1.3) * 35}%`,
                    left: `${10 + Math.cos(i * 1.3) * 35}%`,
        ...
        {/* Level badge */}
        <div
          className={`absolute -bottom-3 left-1/2 -translate-x-1/2 font-bold text-sm px-3 py-0.5
            rounded-full border-2 shadow whitespace-nowrap
            ${isLevelingUp
              ? "bg-yellow-300 border-yellow-500 text-black animate-bounce"
      ...
      <div className="text-center mt-3">
        <p className="text-white font-bold text-lg tracking-widest">{stage.name}</p>
      </div>
